...
install_helm() {
  curl -L https://get.helm.sh/helm-v${helm_version}-linux-arm64.tar.gz -o /tmp/helm.tar.gz \
    && tar -zxvf /tmp/helm.tar.gz -C /tmp \
    && mv /tmp/linux-arm64/helm ${bin_dir}/helm
}

install_kubectl() {
  curl -L https://dl.k8s.io/release/v${kubectl_version}/bin/linux/arm64/kubectl -o ${bin_dir}/kubectl \
